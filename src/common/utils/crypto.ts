...
export class ICrypto {
  ...
  static generateRandomString(length: number = 16): string {
    ...
    return (timestamp + crypto.randomBytes(bytes).toString(ENCODINGS.HEX)).slice(0, length)
  }
   ...
   */
  static generateSecureRandom(size: number = 32, encoding: CryptoEncoding = ENCODINGS.HEX): string {
  ...
  static verifyHmac(
    ...
    return crypto.timingSafeEqual(Buffer.from(newHmac, encoding), Buffer.from(signature, encoding))
  }
  ...
  static aesEncrypt(
    ...
    encoding: CryptoEncoding = ENCODINGS.BASE64
      ...
      // 使用 scrypt 生成密钥
      const keyBuffer = crypto.scryptSync(key, AES_CONFIG.SALT, AES_CONFIG.KEY_SIZE)
      ...
      // 合并加密结果并编码
      return {
        encrypted: Buffer.concat(encryptedBuffers).toString(encoding as BufferEncoding),
  ...
  static aesDecrypt(
    ...
    const encrypted = Buffer.from(encryptedData.encrypted, encoding as BufferEncoding)
